...
def main():
    ...
    if args.env_name.startswith("h1hand-") or args.env_name.startswith("h1-"):
        ...
        eval_envs = envs
        render_env = envs
    elif args.env_name.startswith("VR-"):
        from environments.mjlab_env import MjlabEnv

        # Reuse the "isaaclab" branches below: MjlabEnv exposes the exact same
        # interface (reset/reset_with_critic_obs/step signatures, asymmetric_obs).
        env_type = "isaaclab"
        envs = MjlabEnv(
            args.env_name,
            device.type,

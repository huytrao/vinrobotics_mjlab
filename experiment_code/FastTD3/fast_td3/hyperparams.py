...
def get_args():
    ...
    env_to_args_class = {
        ...
        # vr_mjlab
        "VR-M3-1-Flat": VRM31FlatArgs,
        "VR-M3-1-12DOF-Flat": VRM31TwelveDofFlatArgs,
    }
    ...
    elif base_args.env_name.startswith("VR-"):
        # vr_mjlab
        specific_args = tyro.cli(VrMjlabArgs)
    else:
        # MuJoCo Playground
...
@dataclass
class VrMjlabArgs(IsaacLabArgs):
    """Defaults for vr_mjlab's VR-M3-1 velocity tasks (mjlab: IsaacLab-style manager
    API, but built on MuJoCo Warp rather than IsaacLab). Routed through MjlabEnv,
    which is dispatched in train.py under env_type == "isaaclab"."""

    num_steps: int = 8
    num_updates: int = 4
    # This repo's PPO baseline (docs/training_strategy_2h_fullbody.md) collects
    # num_steps_per_env=32 env-steps per iteration; 2400 iterations of that
    # PPO run is used as the training budget target here (2400 * 32 = 76_800),
    # so a FastTD3 run sees roughly the same amount of environment experience as
    # the 2400-iteration PPO comparison run.
    total_timesteps: int = 76_800


@dataclass
class VRM31FlatArgs(VrMjlabArgs):
    env_name: str = "VR-M3-1-Flat"


@dataclass
class VRM31TwelveDofFlatArgs(VrMjlabArgs):
    env_name: str = "VR-M3-1-12DOF-Flat"
